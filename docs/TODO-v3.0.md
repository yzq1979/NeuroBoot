...
> **Last updated**: 2026-05-25（W5-6 Phase 2 代码完成）
> **Status**: 7/9 模块代码完成（W5-6 P1+P2），19 commits 领先 origin/main（**未 push**），ISO 待重 build
...
| 指标 | v1.0.1（起点）| 当前（W5-6 P2 代码完成）| v3.0 末预期 |
|---|---|---|---|
| 单测 | 64 | **189**（+125；其中 W5-6 P1+P2 共 +36）| ≥ 200 |
| 工具数 | 22 | **30**（+5 W7 + 3 元工具 load_skill / propose_plan / memory = +8 实质，原 22 不变）| ~31 |
| Skill 数 | 3 | **8**（+5 W2-3）| 8（W2-3 后稳定）|
| neuroboot.exe | 12.46 MB | **14.42 MB**（+1.96 MB 累计；rusqlite bundled +1.6 MB + sqlite-vec +160 KB）| ~14.5 MB（稳定）|
| ISO 体积 | 2.93 GB | 3.32 GB（Sprint 1.2 已 build，**未含本会话 19 commits**）| ~3.9 GB（含 RAG GGUF + 全量 db）|
...
**推荐顺序**：W5-6 Phase 3（操作步骤）→ W8 → W8.5

理由：
- **W5-6 Phase 3 (~1-2 小时操作 + ~30 min build)**：不写代码，跑命令。详见上文
  「W5-6 Phase 3 (operational)」节的 7 步清单。下载嵌入模型 + 生成 embed db + 重 build ISO
- **W8 Eval (~5-7 天)** 在 Phase 3 完成后跑最有价值（golden prompts 能测 RAG vec hybrid + memory + hooks 全链路）
- **W8.5 真测** 必须在 W8 完成后

### 2. push 决策

19 commits 都在本地。如果你信任本机不丢，可以等 v3.0 完全 release 再 push。
...
**不含**本会话 19 commits 的改动（W1 / W1.5 / W2-3 / W3-4 / W7 / W6-7 / W5-6 P1+P2）。
...
W5-6 Phase 3 是纯操作步骤（下载嵌入模型 + 跑 embed + 重 build ISO），详见上文
「W5-6 Phase 3 (operational)」节的 7 步清单。建议把这个步骤跟 W8 / W8.5 一起做。
