...
> **Last updated**: 2026-05-25（W6-7 完成）
> **Status**: 6/9 模块完成，15 commits 领先 origin/main（**未 push**），ISO 待重 build
...
| 指标 | v1.0.1（起点）| 当前（W6-7 完成）| v3.0 末预期 |
|---|---|---|---|
| 单测 | 64 | **152**（+88）| ≥ 170 |
| 工具数 | 22 | **30**（+5 W7 + 3 元工具 load_skill / propose_plan / memory = +8 实质，原 22 不变）| ~31 |
| Skill 数 | 3 | **8**（+5 W2-3）| 8（W2-3 后稳定）|
| neuroboot.exe | 12.46 MB | **12.66 MB**（+200 KB W1.5 + W3-4 + W7 + W6-7 累计）| ~13 MB（W5-6 RAG 加 sqlite-vec deps） |
| ISO 体积 | 2.93 GB | 3.32 GB（Sprint 1.2 已 build，**未含本会话 15 commits**）| ~3.9 GB（含 RAG GGUF + db）|
...
- **W5-6 RAG** 是最大块（~2 周），需要单独大会话 + 下载 400 MB embed 模型 + Python 构建。
  W6-7 已经把 memory 工具上线，RAG 落地后 lookup_error_code 内核升级 API 不变
- **W8 Eval** 在 W5-6 完成后跑最有价值（golden prompts 能测 RAG + memory + hooks 全链路）
- **W8.5 真测** 必须在前 2 个都完成后

### 2. push 决策

15 commits 都在本地。如果你信任本机不丢，可以等 v3.0 完全 release 再 push。
...
**不含**本会话 15 commits 的改动（W1 / W1.5 / W2-3 / W3-4 / W7 / W6-7）。
